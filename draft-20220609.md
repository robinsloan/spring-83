...
*Note: this is a draft published in June 2022 for review and comment.*
...
**Predictable.** This means clients only receive the boards they request, when they request them; there is no mechanism by which a server can "push" an unsolicited board.
...
*realm*: a set of peers, listed together in a YAML document reachable on the public internet. The default *realm* is defined at TKTKTK.

## Publishers, keys, and boards

Publishers (who might be people, computer programs, or anything else) are identified and authorized by keypairs on the Ed25519 curve. The public part of the keypair, formatted as 64 hex characters, is the publisher's identifier, used to request their board from a server. The secret part of the keypair allows the publisher alone to edit their board.

Keys are generated randomly, but they must fit a particular format that indicates their years of potential use. Every key's final six hex characters must match the regex

```
/ed[0-9]{4}$/
```

and the final four characters, interpreted as a decimal number, must fall in the range 2022-2099.

Keys are only valid in two years: the year specified in their final four digits, and the previous year. For example, the key
...
In this way, a key's final four digits represent its "expiration date".
...
The requirement also functions as a Hashcash variation, providing a rudimentary form of abuse mitigation.
...
* make available to boards the Spring '83 suite of CSS variables, listed in Discussion TK
...
This check is not applied to keys for which the server already has a board stored. It is also not applied to the test key, listed below.

You can read more about the difficulty factor later in this document.
...
A realm is a set of Spring '83 servers, specified by a YAML document reachable on the public internet. (The format of this document is TK.) There is no automatic or "trustless" way to join a realm; as with BGP, the foundational routing protocol of the internet, you gotta talk to somebody! 
...
To share a board with peers, the server must use a PUT request similar to the one described above. Note the addition of the `Prefer: respond-async` header:
...
```
difficulty_factor = ( num_current_boards / 10_000_000 )**4
```
...
Let's take the example of a server that is already storing 8.5 million boards.

```
difficulty_factor = ( 8_500_000 / 10_000_000 )**4 = 0.52
```

Using that difficuly factor, we can calculate the key threshold:
...
The server will reject PUT requests for new keys that are not less than <an inscrutable gigantic number>.

A client should determine a server's current admission difficulty using a request of this form:
...
Content-Type: text/html;charset=utf-8
Spring-Version: 83
Spring-Difficulty: <admission difficulty>
...
401: Board was submitted without a signature, or without a valid signature.
403: Board was submitted with a signature that does not meet the admission difficulty.
