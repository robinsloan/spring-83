...
If the client discovers more than one `<link rel="next">` element, it must honor the first and ignore the rest.
...
The board must contain only one `<time>` element. If it contains more than one, the server must honor the first and ignore the rest.
